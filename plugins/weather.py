...
class Weather(Plugin):
    ...
    def db_loc(self, who, loc=None):
        ...
        q = self.dong.db.session.query(table).filter(
            getattr(table.c, 'user') == who)
        is_in_db = q.all()
    ...
    def fetch_weather(self, callback, who, loc):
        ...
        try:
            ...
            try:
                data['rain_in'] = float(
                    results['current_observation']['precip_today_in'])
            except BaseException:
                data['rain_in'] = 0.0
            try:
                data['rain_mm'] = float(
                    results['current_observation']['precip_today_metric'])
            except BaseException:
                data['rain_mm'] = 0.0
